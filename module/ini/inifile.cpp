...
bool IniFile::getValue(const char* section, const char* key, std::string& str)
{
	char _buf[1024];
	char _Text[1024];
	if (!getValue(section, key, _buf, sizeof(_buf), _Text, sizeof(_Text)))
		return false;
	str = _Text;
	return true;
}

bool IniFile::getValue(const char* section, const char* key, int8_t& value)
{
	char _buf[1024];
	int _number = 0;
	if (!getValue(section, key, _buf, sizeof(_buf), _number))
		return false;
	value = _number;
	return true;
}

bool IniFile::getValue(const char* section, const char* key, int32_t& value)
{
	char _buf[1024];
	return getValue(section, key, _buf, sizeof(_buf), value);
}

bool IniFile::getValue(const char* section, const char* key, int64_t& value)
{
	char _buf[1024];
	if (!getValue(section, key, _buf, sizeof(_buf)))
		return false;
	value = atoll(_buf);
	return true;
}

bool IniFile::getValue(const char* section, const char* key, uint8_t& value)
{
	char _buf[1024];
	return getValue(section, key, _buf, sizeof(_buf), value);
}

bool IniFile::getValue(const char* section, const char* key, uint32_t& value)
{
	char _buf[1024];
	int _number = 0;
	if (!getValue(section, key, _buf, sizeof(_buf), _number))
...
bool IniFile::getValue(const char* section, const char* key, float& value)
{
	char _buf[1024];
	float _number = 0;
	if (!getValue(section, key, _buf, sizeof(_buf), _number))
