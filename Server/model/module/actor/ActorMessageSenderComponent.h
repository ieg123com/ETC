...
public:
	...
	template<typename T>
	std::shared_ptr<T> Call(const int64_t actor_id, const std::shared_ptr<IActorRequest>& request, const bool need_exception = true) {
		if (actor_id == 0)
		...
		return std::static_pointer_cast<T>(__Call(actor_id, rpc_id, request,need_exception));
	}

	std::shared_ptr<IActorResponse> Call(const int64_t actor_id, const std::shared_ptr<IActorRequest>& request, const bool need_exception = true) {
		if (actor_id == 0)
			throw std::exception(std::format("actor id is 0 : %s", request->GetTypeName().c_str()).c_str());
		int32_t rpc_id = __GetRpcId();
		request->SetRpcId(rpc_id);
		return __Call(actor_id, rpc_id, request,need_exception);
	}
...
private:
	...
	// ActorMessage 消息调用
	std::shared_ptr<IActorResponse> __Call(const int64_t actor_id, const int32_t rpc_id, const std::shared_ptr<IMessage>& message,const bool need_exception);
