...
public:
	...
	static std::shared_ptr<T> CreateResponseTry(const std::shared_ptr<IMessage>& message, const ETC_ERR error) {
		...
		{
			auto request = dynamic_cast<IActorMessage*>(message)
		...
		{
			auto request = dynamic_cast<IActorRequest*>(message)
			actor_req = __CreateResponseTry(request->GetOpcode(), request->GetRpcId(), error);
		}
		break;
		default:
			throw std::exception(std::format("没有找到这个消息的 actor request 类型，%s : %s", message->GetType().full_name(), ToString(message->GetMessageType()));
			break;
		}
		if (auto response = std::dynamic_pointer_cast<T>(actor_req))
			return response;
		throw std::exception(std::format("无法转换回复消息类型 %s => %s", actor_req->GetType().full_name(), typeof(T).full_name()).c_str());
	}
