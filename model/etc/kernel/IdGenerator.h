...
namespace Model
{
	...
	struct InstanceIdStruct
	{
		...
		InstanceIdStruct(const InstanceID id) {
			int64_t result = id;
	...
	public:
		...
		static InstanceID GenInstanceId();
		static ID GenId();
	...
	inline InstanceID IdGenerator::GenInstanceId() {
		return IdGenerator::Instance->GenerateInstanceId();
	}

	inline ID IdGenerator::GenId() {
